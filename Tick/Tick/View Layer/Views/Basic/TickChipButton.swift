...
class TickChipButton: TickUIView {
    
    private let container = TickView()
    private let button = TickControl()
    private let imageView = TickImage()
    private var onTap: (() -> Void)? = nil
    private var imageHorizontalPadding = TickDimensions.chipPaddingHorizontal
    private var imageVerticalPadding = TickDimensions.chipPaddingVertical
