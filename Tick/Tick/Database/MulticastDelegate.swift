...
class MulticastDelegate<T> {
    ...
    func addDelegate(_ delegate: T) {
        ...
        self.delegates.insert(WeakObjectWrapper(value: delegateObject))
    }
    
    func removeDelegate(_ delegate: T) {
        let delegateObject = delegate as AnyObject
        self.delegates.remove(WeakObjectWrapper(value: delegateObject))
    }
    
    func invoke(invocation: (T) -> ()) {
        self.delegates.forEach { (delegateWrapper) in
